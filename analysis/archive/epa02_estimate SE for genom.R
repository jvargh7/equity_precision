...
summary_master <- readRDS(paste0(path_ep_folder,"/working/summary genom.RDS")) %>% 
  rename(outcome = genom)

summary_master %>% group_by(outcome) %>% summarize(n = sum(n))
# > 720/(720+966)
# [1] 0.4270463

gen_row = generate_dataset(summary_master,"Rest of World",t0 = round(181813*(0.427046)))
...
data.frame(
  ...
  mutate(prop_correct = total_correct/(197570*(0.427046)),
